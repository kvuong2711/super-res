...
class ZSSR:
    ...
    def __init__(self, input_img, ref_imgs, conf=Config(), ground_truth=None, kernels=None):
        ...
        self.input_test = self.input

        # Read ref images
        self.refs = [ref_img if type(ref_img) is not str else img.imread(ref_img) for ref_img in ref_imgs]
    ...
    def run(self):
        for i, ref in enumerate(self.refs):
            print('** Start training for ref=', i)
            # # Train on all the hr_fathers first
            ...
            for self.sf_ind, (sf, self.kernel) in enumerate(zip(self.conf.scale_factors, self.kernels)):
                ...
                if self.conf.init_net_for_each_sf == True:
                    print('reinit net for each sf')
                    self.model = simpleNet(self.Y)
                if self.cuda and not next(self.model.parameters()).is_cuda:
    ...
    def train(self):
        ...
        for self.iter in range(self.conf.max_iters):
            ...
            # print('hr_father shape:', self.hr_father.shape)

            # Get lr-son from hr-father
            self.lr_son = self.father_to_son(self.hr_father)
            # print('lr_son shape:', self.lr_son.shape)
    ...
    def final_test(self):
        ...
        print('almost_final_sr shape:', almost_final_sr.shape)
    ...
    def plot(self):
        ...
        # trranspose dimension of train_output
        self.train_output = np.transpose(self.train_output, (1, 2, 0))
